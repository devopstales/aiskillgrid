...
**Field IDs are instance-specific.** The placeholders below (`{{TEAM_FIELD}}`, `{{DESCR_FIELD}}`, `{{PROJECT_KEY}}`) stand for the instance's custom fields and project key — read them from the project's `docs/skillgrid/agents/issue-tracker.md` (written by `sdd-onboard` (init)) or discover them with `jira issue view <existing-key>`. Never hardcode IDs from another instance.
