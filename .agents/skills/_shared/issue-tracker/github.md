...
When `force_ticket_creation` is `true`, the `issue-creation` skill MUST be invoked to create the ticket for the `change.md` and `tasks.md` artifacts at the `sdd-propose` and `sdd-spec` phases. Write the resulting `#<n>` into `change.md` **`Ticket:`**.

### Ticket lifecycle (mandatory when `Ticket:` is set)

| Phase | Tracker action |
|---|---|
| **`sdd-apply`** | Keep issue open; every step commit uses `Refs #<n>`; optional progress comment |
| **`sdd-verify`** | Comment verify verdict |
| **`sdd-archive`** | Closing commit / PR uses `Closes #<n>` (or `gh issue close`); ticket close is part of archive, not optional |
