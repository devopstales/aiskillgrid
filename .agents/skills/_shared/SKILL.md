...
- `conventions/` — shared contract documents every SDD skill must honor:
  ...
  - [conventions/mnemonic-code-indexing.md](conventions/mnemonic-code-indexing.md) — Orientation Ladder (`code_status` → `code_map` → symbols/outline → related → `code_read`), Index Freshness, Search Intent Router, config, and gotchas (entry point: the `mnemonic` skill).
