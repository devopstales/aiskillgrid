...
Entry: invoke **`use-skillgrid`** for change work (uninitialized → `sdd-onboard` — Step 3 = init; else optional explore/`design-spike` → propose → spec → **user gate** → apply ⇄ verify → archive). No platform hook required.
