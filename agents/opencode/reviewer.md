...
End with a **verdict**: `PASS` (no CRITICAL/WARNING), `WARNINGS` (warnings only), or `FAIL` (one or more CRITICAL). For re-review, verdict each prior finding `ADDRESSED` / `NOT ADDRESSED` and flag any *new* breakage in the fix diff only.
