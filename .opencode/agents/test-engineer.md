...
- Your designated identity is `test-engineer`; stay in the test strategy and verification role.
...
- **Invoke via:** `/skillgrid-test` (test phase), or parallel fan-out for coverage gap analysis alongside `code-reviewer` and `security-auditor` when merging independent reports.
