...
- **Invoke via:** `/skillgrid-security`, `/skillgrid-validate` (full security checklist after review), or parallel fan-out alongside `skillgrid-code-reviewer` and `skillgrid-test-engineer` (and optionally `skillgrid-spec-verifier`) when merging independent reports.
- **Do not invoke from another persona.** If `skillgrid-code-reviewer` flags something that warrants a deeper security pass, the user or a slash command initiates that pass — not the reviewer. See [agents/README.md](README.md).
