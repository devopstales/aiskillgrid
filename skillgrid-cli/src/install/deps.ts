...
/** Hub MCP includes `trivy` (no subset, or subset lists `trivy`) — matches install.sh mcp_trivy_is_selected. */
export function mcpTrivyIsSelected(mergeMcp: boolean, filterKeys: string[] | null): boolean {
  if (!mergeMcp) return false;
  if (filterKeys == null || filterKeys.length === 0) return true;
  return filterKeys.includes("trivy");
}

function ideSelected(
  selectedIdes: IdeId[],
  ideFlag: string,
  allIdes: boolean,
  selectedTools: OptionalToolId[],
  mcpTrivySelected: boolean,
): boolean {
  if (allIdes) return true;
  if (ideFlag === "openspec" && toolIsSelected(selectedTools, "openspec")) return true;
  if ((ideFlag === "trivy" || ideFlag === "trivy-mcp") && mcpTrivySelected) return true;
...
export function countMissingDeps(
  ...
  const pip: string[] = [];
  const npm: string[] = [];
  const mcpTrivy = mcpTrivyIsSelected(mergeMcp, mcpKeyFilter);
  ...
  for (const dep of IDE_DEPENDENCIES) {
    if (!dep.checkCmd) continue;
    if (!ideSelected(selectedIdes, dep.ideFlag, allIdes, selectedTools, mcpTrivy)) continue;
    if (!shCheck(dep.checkCmd)) {
      if (dep.brew) brew.push(dep.brew);
      else if (dep.pip) pip.push(dep.pip);
      else if (dep.npm) npm.push(dep.npm);
...
export function showDependencies(
  ...
  for (const dep of IDE_DEPENDENCIES) {
    if (!dep.checkCmd) continue;
    if (!ideSelected(selectedIdes, dep.ideFlag, allIdes, selectedTools, mcpTrivy)) {
