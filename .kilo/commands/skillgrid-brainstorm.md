...
7. **Handoff** — Output should feed **`/skillgrid-plan`**, not a full locked spec. Optional: if the team uses a dated design doc path (similar to superpowers’ `docs/.../YYYY-MM-DD-<topic>-design.md`), only write it when the user asks; otherwise keep artifacts light until planning.
...
- Prior art for question style and gates: [superpowers **brainstorming** skill](https://github.com/obra/superpowers/blob/main/skills/brainstorming/SKILL.md) (Skillgrid steps and handoffs above take precedence).
