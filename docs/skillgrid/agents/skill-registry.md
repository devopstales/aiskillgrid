...
| dispatching-parallel-agents | **retired** → redirect to `subagent-execution` (`## Parallel investigation`). | `.agents/skills/dispatching-parallel-agents/SKILL.md` | project |
...
| mnemonic | ALWAYS ACTIVE — Mnemonic memory, code index, and web cache protocol: proactive mem_save, code ladder, search router, sessions (folded: former `mnemonic-code-index`) | `.agents/skills/mnemonic/SKILL.md` | project |
| mnemonic-code-index | **retired** → redirect to `mnemonic` (Step 5 + `_shared/conventions/mnemonic-code-indexing.md`). | `.agents/skills/mnemonic-code-index/SKILL.md` | project |
...
Phase order (v4): `onboard → propose → spec → apply ⇄ verify → archive` (explore/spike are pre-propose helpers).
