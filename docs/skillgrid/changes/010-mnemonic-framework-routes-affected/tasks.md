...
**Architecture:** A third extractor layer (framework-route per language/framework) adds `route` nodes + `references`/`navigates` edges into 005's existing graph. `code_affected` is a pure edge-traversal query (no new extraction) over 005's import/`tests-for` edges; `code_rename` is the write counterpart splitting edits into graph-confidence vs. text-search buckets with `dry_run`. An fsnotify watcher in the serve/MCP path publishes via **copy-and-swap** (sidecar build → atomic publish) so readers never see a torn index; running processes reopen the new index on the next tool call without restart, and a warm embedder daemon keeps 005's ONNX model in RAM (load-once, idle-evict, MCP-heartbeat keep-alive).
...
| Field | Value |
|-------|-------|
| Estimated changed lines (change) | ~2200–3000 |
| 400-line budget risk | High |
| Chained PRs recommended | Yes |
| Delivery strategy | ask-on-risk |

Honest forecast: three vertical slices (routes → affected/rename → watcher+embedder), each likely its own stacked PR. Step 03 is the largest (~800–1100) and should land on its own. Do not attempt a single-PR delivery without explicit exception.
...
Additive `013_*` schema + framework-route extractor (route + references + navigates edges) so URL→handler and screen→screen become one graph hop, while 005/008 tools stay unchanged.

### Out of scope / Non-Goals

- `code_affected` / `code_rename` (step 02)
- Watcher / staleness banner / warm embedder (step 03)
...
- Produces: `route` nodes + `references`/`navigates` edge types in the existing graph (additive `013_*`); per-framework node maps (routes→handler `references`, routers→screen `navigates`); same-tx route-extraction hook in `Indexer.Run`; route/navigates query MCP tools
...
- [ ] 01.4 `[AFK]` Malformed routing file falls back and index continues (Scenario: Malformed routing file falls back and index continues) — `Run: go test ./skillgrid-cli/internal/mnemonic/route/... ./skillgrid-cli/internal/mnemonic/codeindex/... -count=1` — Expected: PASS
- [ ] 01.5 `[AFK]` Framework with no routes reports none (Scenario: Framework with no routes reports none) — `Run: go test ./skillgrid-cli/internal/mnemonic/route/... -count=1` — Expected: PASS
- [ ] 01.6 `[RED]` Mnemonic tool surface — route tools register, `code_search` stable, bad args rejected (Scenario: Route tools register and code_search stays stable) — threat: Mnemonic tool surface
  - [ ] 01.6.a Write failing test — assert route/navigates query tools register with distinct `code_*` names; `code_search` name + required `query` param schema unchanged; 005/008 tools intact; bad route args rejected with a clear validation error
  - [ ] 01.6.b Run to confirm fail — `Run: go test ./skillgrid-cli/internal/mnemonic/mcp/... -run RouteTools -count=1` — Expected: FAIL
  - [ ] 01.6.c Minimal implementation — `tools_code_route.go` + server registration without dropping existing `code_*`
  - [ ] 01.6.d Run to confirm pass — `Run: go test ./skillgrid-cli/internal/mnemonic/mcp/... -run RouteTools -count=1` — Expected: PASS
  - [ ] 01.6.e Commit — `feat(mnemonic): register route and navigates query tools`
...
| Runtime harness | `skillgrid index` on a fixture web app; query route→handler→screen via SQL / MCP | PASS | | |
| Rollback boundary | Drop `013_*` + `route/` + `tools_code_route.go`; revert indexer hook | PASS | | |
| Global Constraints | — | held | | |

### Commit

When step DoD is met: `feat(mnemonic): framework route nodes and navigates edges`
...
`code_affected` (read) + `code_rename` (write) MCP/CLI tools — "which tests do I run after this change?" and "what does renaming this symbol touch?" — both grounded in the existing graph.

### Out of scope / Non-Goals

- Route edges (step 01); watcher / banner / warm embedder (step 03)
...
- Modify: `skillgrid-cli/cmd/skillgrid/code_intel.go` (`skillgrid search affected` with `--stdin`/`--depth`/`--filter`/`--json`/`--quiet` + `skillgrid search rename`)
- Test: `skillgrid-cli/internal/mnemonic/affected/...`, `skillgrid-cli/internal/mnemonic/mcp/...`, `skillgrid-cli/cmd/skillgrid/...`

**Interfaces:**
- Consumes: 005's import + `tests-for` edges and symbol disambiguation; 005/010 `symbols`/`edges` tables
- Produces: `code_affected` traversal (depth-capped, `--stdin`/`--filter`/`--json`/`--quiet`); `code_rename` (graph vs. text-search edit buckets, `dry_run` default true, per-edit confidence); MCP tools + CLI parity
...
- [ ] 02.3 `[RED]` `code_rename` splits edits into graph vs text-search buckets (Scenario: code_rename returns graph and text-search edit buckets)
  ...
  - [ ] 02.3.b Run to confirm fail — `Run: go test ./skillgrid-cli/internal/mnemonic/affected/... -run CodeRename -count=1` — Expected: FAIL
  - [ ] 02.3.c Minimal implementation — `affected/rename.go` (resolve via 005 disambiguation; graph edges → graph bucket; string matches → text-search bucket; per-edit confidence; `dry_run` default true)
  - [ ] 02.3.d Run to confirm pass — `Run: go test ./skillgrid-cli/internal/mnemonic/affected/... -run CodeRename -count=1` — Expected: PASS
  - [ ] 02.3.e Commit — `feat(mnemonic): code_rename graph and text-search buckets`
- [ ] 02.4 `[RED]` PR commands — non-dry `code_rename` edits only listed files, no commit/push (Scenario: code_rename applies only listed files without committing) — threat: PR commands
  - [ ] 02.4.a Write failing test — a non-dry `code_rename` against a fixture edits exactly the files in its plan (no other file touched); no commit or push is performed
  - [ ] 02.4.b Run to confirm fail — `Run: go test ./skillgrid-cli/internal/mnemonic/affected/... -run CodeRenameApply -count=1` — Expected: FAIL
  - [ ] 02.4.c Minimal implementation — apply path limited to planned files; no git side effects
  - [ ] 02.4.d Run to confirm pass — `Run: go test ./skillgrid-cli/internal/mnemonic/affected/... -run CodeRenameApply -count=1` — Expected: PASS
  - [ ] 02.4.e Commit — `feat(mnemonic): code_rename applies only planned files`
- [ ] 02.5 `[AFK]` Ambiguous rename target returns a ranked candidate list (Scenario: Ambiguous rename target returns candidates) — `Run: go test ./skillgrid-cli/internal/mnemonic/affected/... -run CodeRenameAmbiguous -count=1` — Expected: PASS
- [ ] 02.6 `[AFK]` No changed files gives an empty result not an error (Scenario: code_affected with no changed files returns empty) — `Run: go test ./skillgrid-cli/internal/mnemonic/affected/... -count=1` — Expected: PASS
- [ ] 02.7 `[RED]` Mnemonic tool surface — `code_affected` + `code_rename` register, rename disambiguation, dry_run touches nothing, 005/008 tools stable (Scenario: affected and rename tools register and stay stable) — threat: Mnemonic tool surface
  - [ ] 02.7.a Write failing test — assert `code_affected` + `code_rename` register with distinct `code_*` names; rename disambiguation returns a candidate list (no silent pick); `dry_run` touches nothing; 005/008 tools (incl. `code_search`) still stable; bad affected/rename args rejected with a clear validation error
  - [ ] 02.7.b Run to confirm fail — `Run: go test ./skillgrid-cli/internal/mnemonic/mcp/... -run AffectedTools -count=1` — Expected: FAIL
  - [ ] 02.7.c Minimal implementation — `tools_code_affected.go` + server registration without dropping existing `code_*`
  - [ ] 02.7.d Run to confirm pass — `Run: go test ./skillgrid-cli/internal/mnemonic/mcp/... -run AffectedTools -count=1` — Expected: PASS
  - [ ] 02.7.e Commit — `feat(mnemonic): register code_affected and code_rename tools`
...
| Runtime harness | `git diff --name-only \| skillgrid search affected --stdin` on fixture repo; `skillgrid search rename` dry-run | PASS | | |
| Rollback boundary | Drop `affected/` + `tools_code_affected.go`; revert `code_intel.go` | PASS | | |
| Global Constraints | — | held | | |

### Commit

When step DoD is met: `feat(mnemonic): code_affected and code_rename tools`
...
fsnotify watcher + copy-and-swap publication + reader auto-reopen + connect-time catch-up + staleness banner + warm embedder daemon — keep the graph fresh *and* the embedder warm as the agent edits.

### Out of scope / Non-Goals

- Route edges (step 01); `code_affected`/`code_rename` (step 02)
...
- Modify: `skillgrid-cli/cmd/skillgrid/main.go` (`serve`/MCP watcher + warm-embedder wiring + `skillgrid index status` pending-sync section)
...
- [ ] 03.7 `[AFK]` Watcher reindex failure stops in-place, keeps old on swap (Scenario: Reindex failure keeps the old index live) — `Run: go test ./skillgrid-cli/internal/mnemonic/codeindex/... -run WatchFailure -count=1` — Expected: PASS
- [ ] 03.8 `[AFK]` Watcher disabled means manual index (Scenario: Watcher disabled means manual index) — `Run: go test ./skillgrid-cli/internal/mnemonic/codeindex/... ./skillgrid-cli/cmd/skillgrid/... -run WatchDisabled -count=1` — Expected: PASS
- [ ] 03.9 `[RED]` Warm embedder lifecycle — load-once, reused across search calls (Scenario: Warm embedder loads once and reuses) — threat: Warm embedder lifecycle
  - [ ] 03.9.a Write failing test — the ONNX model loads once and is reused across `skillgrid search` / MCP calls (no per-call model-load; the ~270MB model is not re-loaded per invocation); the first call pays the load, subsequent calls do not
  - [ ] 03.9.b Run to confirm fail — `Run: go test ./skillgrid-cli/internal/mnemonic/embedder/... -run WarmEmbedder -count=1` — Expected: FAIL
  - [ ] 03.9.c Minimal implementation — `embedder/warm.go` load-once ONNX cache + `mcp/server.go` / `main.go` wiring into the search path
  - [ ] 03.9.d Run to confirm pass — `Run: go test ./skillgrid-cli/internal/mnemonic/embedder/... -run WarmEmbedder -count=1` — Expected: PASS
  - [ ] 03.9.e Commit — `feat(mnemonic): warm embedder load-once cache`
- [ ] 03.10 `[RED]` Warm embedder lifecycle — evicts after idle_timeout, heartbeat keeps alive (Scenario: Warm embedder evicts on idle and survives heartbeat) — threat: Warm embedder lifecycle
  - [ ] 03.10.a Write failing test — after `idle_timeout` (default 10min, tunable) of no use the warm embedder is evicted from RAM; an active MCP client connection keeps it alive via heartbeat (no eviction while connected); `off`/external providers hold no RAM
  - [ ] 03.10.b Run to confirm fail — `Run: go test ./skillgrid-cli/internal/mnemonic/embedder/... ./skillgrid-cli/internal/mnemonic/mcp/... -run WarmEvict -count=1` — Expected: FAIL
  - [ ] 03.10.c Minimal implementation — `idle_timeout` evict timer + MCP-heartbeat keep-alive in `embedder/warm.go` + `server.go`
  - [ ] 03.10.d Run to confirm pass — `Run: go test ./skillgrid-cli/internal/mnemonic/embedder/... ./skillgrid-cli/internal/mnemonic/mcp/... -run WarmEvict -count=1` — Expected: PASS
  - [ ] 03.10.e Commit — `feat(mnemonic): warm embedder idle-evict and heartbeat keep-alive`
- [ ] 03.11 `[AFK]` Absent or failed model degrades to FTS and signals (Scenario: Absent model degrades to FTS and signals) — `Run: go test ./skillgrid-cli/internal/mnemonic/embedder/... ./skillgrid-cli/internal/mnemonic/mcp/... -run WarmFallback -count=1` — Expected: PASS
- [ ] 03.12 `[AFK]` Evicted-then-reused embedder reloads transparently (Scenario: Evicted embedder reloads on reuse) — `Run: go test ./skillgrid-cli/internal/mnemonic/embedder/... -run WarmReload -count=1` — Expected: PASS
- [ ] 03.13 `[AFK]` `skillgrid index status` pending-sync section (Scenario: index status shows pending sync) — `Run: go test ./skillgrid-cli/cmd/skillgrid/... -run IndexStatus -count=1` — Expected: PASS
- [ ] 03.14 `[RED]` Mnemonic tool surface — watcher alters no tool schemas, banner present, bad args rejected (Scenario: Watcher keeps tool schemas stable) — threat: Mnemonic tool surface
  - [ ] 03.14.a Write failing test — assert the watcher + banner do not alter any existing 005/008/new `code_*` tool schemas; the staleness banner is present on the response path; bad watcher/status args are rejected with a clear validation error
  - [ ] 03.14.b Run to confirm fail — `Run: go test ./skillgrid-cli/internal/mnemonic/mcp/... ./skillgrid-cli/cmd/skillgrid/... -run WatcherSurface -count=1` — Expected: FAIL
  - [ ] 03.14.c Minimal implementation — schema-stability assertions + banner/arg-validation wiring
  - [ ] 03.14.d Run to confirm pass — `Run: go test ./skillgrid-cli/internal/mnemonic/mcp/... ./skillgrid-cli/cmd/skillgrid/... -run WatcherSurface -count=1` — Expected: PASS
  - [ ] 03.14.e Commit — `feat(mnemonic): watcher tool-surface stability and status`
...
| Runtime harness | `skillgrid serve` + edit a file mid-session; confirm auto-reopen + banner; `skillgrid index status` pending section; `SKILLGRID_NO_WATCH=1` graceful | PASS | | |
| Rollback boundary | Drop `watch.go`/`publish.go`/`warm.go` + banner/heartbeat/auto-reopen in serve path; revert `go.mod` + `main.go` | PASS | | |
| Global Constraints | — | held | | |

### Commit

When step DoD is met: `feat(mnemonic): auto-sync watcher, copy-and-swap, and warm embedder`
