...
| DEFINE | `/skillgrid-init` | [skillgrid-init.md](../.cursor/commands/skillgrid-init.md) | **`.skillgrid/config.json`** (ticketing + **artifact store** `hybrid` strongly recommended \| `openspec` \| `engram` + configurable **PRD workflow**), greenfield/brownfield, **OpenSpec** bootstrap when mode includes disk, Engram when mode includes memory, graphify, OpenCode, **`.skillgrid/`** tree. |
...
| DEFINE | `/skillgrid-design` | [skillgrid-design.md](../.cursor/commands/skillgrid-design.md) | On-demand UI/UX workshop for design extraction, browsing, tuning, options, previews, audits, and root `DESIGN.md` updates. |
