...
- `/skillgrid-brainstorm` clarifies goals and compares possible approaches.
...
- `/skillgrid-plan` creates the product intent and technical change structure.
- `/skillgrid-breakdown` turns the plan into ordered tasks, testable slices, and `[HITL]` or `[AFK]` labels.
...
- `/skillgrid-loop` advances one safe phase or one safe `[AFK]` slice, then reassesses.
...
- `/skillgrid-finish` archives or syncs specs, updates status, prepares git or PR handoff, and checks docs and evidence.
